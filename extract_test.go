...
func TestDestroyer(t *testing.T) {
	d := ExtractDestroyer(extractHtml)
	if bytes.Compare(d, []byte(`GuristaPrime`)) != 0 {
		t.Errorf("Expected GuristaPrime, got: %s", string(d))
	}
}

func TestLinks(t *testing.T) {
	urls := ExtractLinks(extractHtml)
	for i, u := range urls {
		t.Logf("[%d] %s", i, u)
	}
}
